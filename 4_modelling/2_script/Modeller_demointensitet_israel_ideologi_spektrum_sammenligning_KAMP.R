...
run_model <- function(data, ideology_filter = NULL, comp_filter = NULL, model_name) {
  ...
  # Udtræk koefficient for demonstration_intensity_israel
...
israel_plot <- ggplot(results_df, aes(x = group, y = estimate, shape = category)) +
  ...
  theme(
    ...
    legend.position = "bottom",             # Flyt legend til toppen
...
# 1) Omnavngiv og re‐ordn kolonner til LaTeX‐tabel
table_df <- results_df %>%
  dplyr::select(
    Model    = model,
    Gruppe   = group,
