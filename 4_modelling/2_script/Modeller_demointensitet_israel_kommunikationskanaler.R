...
run_model <- function(data, communication_var, ideology_filter, model_name) {
  ...
  model <- tryCatch({
    femlm(
      as.formula(paste(communication_var, "~ demonstration_intensity_israel + demonstration_intensity_total_7_after | bioguide_id + date")),
      data = subset_data,
      family = "negbin"
    )
...
final_plot_israel <- ggplot(results_df, aes(
  x = ideology_group, 
  y = estimate, 
  ymin = conf_low, 
  ymax = conf_high, 
...
# Display the plot
print(final_plot_israel)

# Gemmer figuren
ggsave(
  filename = "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/4_modelling/3_output/plots_kommunikation/pro_israel_kommunikation.pdf",
  plot = final_plot_israel,
  width = 12, height = 6
)
...
wide_df %>%
  kable(
    format    = "latex",
    booktabs  = TRUE,
    caption   = "Effektestimater (±95 \\% CI) for demonstrationintensitet på forskellige kommunikationskanaler efter ideologisk gruppe",
    label     = "tab:B1",
    digits    = 3,
    escape    = FALSE,
    align     = c("l", rep("r", 8))
  ...
  add_header_above(c(
    " " = 1,
    "Samlet kommunikation"       = 2,
    "Talerstolen"                = 2,
    "Nyhedsbreve"                = 2,
    "Officielle udtalelser"      = 2
  )) %>%
