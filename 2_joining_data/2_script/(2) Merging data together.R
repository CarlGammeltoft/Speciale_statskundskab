...
CCC_summarized <- CCC_filtered %>% 
  ...
  summarize(
    # Total protests
    count_protest = sum(total_protest == 1, na.rm = TRUE),
    count_protest_size = sum(size_mean, na.rm = TRUE),
    count_non_normative = sum(non_normative, na.rm = TRUE),
    count_violent = sum(violent, na.rm = TRUE),
    
    # Palestine protests
    count_palestine_protests = sum(palestine_protest == 1, na.rm = TRUE),
    size_palestine_protests = sum(size_mean * (palestine_protest == 1), na.rm = TRUE),
    non_normative_palestine_protests = sum(non_normative * (palestine_protest == 1), na.rm = TRUE),
    violent_palestine_protests = sum(violent * (palestine_protest == 1), na.rm = TRUE),
    
    # Israel protests
    count_israel_protests = sum(israel_protest == 1, na.rm = TRUE),
    size_israel_protests = sum(size_mean * (israel_protest == 1), na.rm = TRUE),
    non_normative_israel_protests = sum(non_normative * (israel_protest == 1), na.rm = TRUE),
    violent_israel_protests = sum(violent * (israel_protest == 1), na.rm = TRUE),
    
    # Neutral protests
    count_neutral_protests = sum(neutral_protest == 1, na.rm = TRUE),
    size_neutral_protests = sum(size_mean * (neutral_protest == 1), na.rm = TRUE),
    non_normative_neutral_protests = sum(non_normative * (neutral_protest == 1), na.rm = TRUE),
    violent_neutral_protests = sum(violent * (neutral_protest == 1), na.rm = TRUE),
...
# Laver CCC-Summarized klar, så jeg kan gemme den
CCC_summarized_save <- CCC_summarized %>% 
  filter(!(is.na(bioguide_id)))
...
vars_zero <- c("count_protest", "count_statement", "sum_legislative_discussion", 
               ...
               "sum_newsletters", "sum_floor", "sum_source_statements", "count_violent",
               "count_palestine_protests",
               "size_palestine_protests",
               "non_normative_palestine_protests",
               "violent_palestine_protests",
               "count_israel_protests",
               "size_israel_protests",
               "non_normative_israel_protests",
               "violent_israel_protests",
               "count_neutral_protests",
               "size_neutral_protests",
               "non_normative_neutral_protests",
               "violent_neutral_protests")
...
data_full <- data_full %>%
  ...
  replace_na(list(count_protest = 0, 
                  ...
                  count_palestine_protests = 0,
                  size_palestine_protests = 0,
                  non_normative_palestine_protests = 0,
                  violent_palestine_protests = 0,
                  count_israel_protests = 0,
                  size_israel_protests = 0,
                  non_normative_israel_protests = 0,
                  violent_israel_protests = 0,
                  count_neutral_protests = 0,
                  size_neutral_protests = 0,
                  non_normative_neutral_protests = 0,
                  violent_neutral_protests = 0
                  )) %>%
...
Profiles_selected <- Profiles %>% 
  dplyr::select(bioguide_id, 
                ...
                ideology_ideology,
                communication_count,
                ideology_ideology_rank_lib,
                ideology_ideology_rank_con,
                ideology_ideology_percentile_lib,
                ideology_ideology_percentile_con,
                ideology_percentile,
                ideology_rank
                )
...
# Erstatter NA's med 0 for komite-data
data_full_clean <- data_full_clean %>% 
  mutate(relevant_committee = ifelse(is.na(relevant_committee), 0, data_full_clean$relevant_committee))
...
write.csv(data_full_clean, "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/3_finalizing/1_input/data_full.csv")
write.csv(data_full_clean, "/Users/carlgammeltoft/Documents/R_speciale_løbende/data_04122024.csv")

# Gemmer den
write.csv(PRL_summarized, "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/3_finalizing/1_input/PRL_summarized.csv")
dput(data, "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/3_finalizing/1_input/CCC_filtered.txt")
