...
combined_data <- data %>%
  ...
  filter(!is.na(communication_window_21)) %>% # Remove NA in communication_window_21 but keep all groups
  mutate(
    Group = case_when(
  ...
  summarise(
    MeanCommunication = mean(communication_window_21, na.rm = TRUE),
    .groups = "drop"
  )
...
gnm_21_pal_plot <- ggplot(combined_data, aes(x = group, y = MeanCommunication, fill = Group)) +
  ...
  labs(
    ...
    x = "Ideologisk gruppe",
    y = "Gennemsnitlig kommunikation (21 dage)",
...
combined_israel_data <- data %>%
  ...
  filter(!is.na(communication_window_21)) %>% # Fjern NA i kommunikation
  mutate(
    Group = case_when(
...
all_politicians_israel <- data %>%
  ...
  filter(!is.na(communication_window_21)) %>%
  group_by(Group) %>%
  summarise(
...
gnm_21_isr_plot <- ggplot(combined_israel_data, aes(x = group, y = MeanCommunication, fill = Group)) +
  # Baggrundsfarve til "Alle politikere"
  annotate("rect", xmin = 4.5, xmax = 5.5, ymin = -Inf, ymax = Inf, fill = "lightgrey", alpha = 0.8) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_hline(yintercept = 0) +
...
legend <- get_single_legend(
  gnm_21_pal_plot + theme(legend.position = "bottom")
)
...
ggsave(
  filename = "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/4_modelling/3_output/plots_deskriptiv_sammenligning_kongruente_politikere_21dage/barplots_combined_21_days.png",
  plot = final_plot,
  width = 10, height = 8
)
...
communication_pale_data <- data %>%
  ...
  summarise(
    mean_communication_with_pale_demo = mean(communication_window_21[count_palestine_protests > 0], na.rm = TRUE),
    mean_communication_without_pale_demo = mean(communication_window_21[count_palestine_protests == 0], na.rm = TRUE),
...
gnm_pal_21_ideo_plot <- ggplot() +
  ...
  labs(
    ...
    y = "Forskel i kommunikation (21 dage)"
  ) +
...
communication_isr_data <- data %>%
  group_by(ideology_percentile_rep, bioguide_id) %>%
  summarise(
    mean_communication_with_isr_demo = mean(communication_window_21[count_israel_protests > 0], na.rm = TRUE),
    mean_communication_without_isr_demo = mean(communication_window_21[count_israel_protests == 0], na.rm = TRUE),
    .groups = "drop" # Remove grouping to avoid unintended behavior
  ) %>%
  pivot_longer(
...
gnm_pal_21_ideo_plot <- gnm_pal_21_ideo_plot +
  labs(title = NULL) +  # Fjern overskrift
  theme(plot.title = element_blank())  # Sørg for ingen titel vises

gnm_isr_21_ideo_plot <- gnm_isr_21_ideo_plot +
  labs(title = NULL) +  # Fjern overskrift
  theme(plot.title = element_blank())  # Sørg for ingen titel vises

# Kombiner ideologi-plots
combined_ideoplots <- plot_grid(
  gnm_pal_21_ideo_plot,  # Første plot (Pro-palæstinensisk)
  gnm_isr_21_ideo_plot,  # Andet plot (Pro-israelsk)
...
ggsave(
  filename = "/Users/carlgammeltoft/Lokale dokumenter/R_speciale/4_modelling/3_output/plots_deskriptiv_sammenligning_kongruente_politikere_21dage/ideoplots_combined_21_days.png",
